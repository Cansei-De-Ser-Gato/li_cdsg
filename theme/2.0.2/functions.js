...
theme.generateContent.form_rastreio = function(prop,oObj){
    ...
    $('body').on('submit', '[apx_load="form_rastreio"] form', function(e){
        ...
        window.open('https://app.melhorrastreio.com.br/app/correios/' + code, '_blank');
    });
    return el.prop('outerHTML');
}
...
theme.generateContent.checkout_steps = function(prop, oObj){
    ...
    el.append('<div class="item '+ ($('.pagina-pedido-finalizado').length > 0 ? 'active' : '') +'"><span><i></i></span><strong>Pagamento</strong></div>');
